...
char * Socket::Create(int af, int type, int proto)
{
        ...
        if (sock == INVALID_SOCKET) {
                sprintf_s(mess, "Error - %s - in [Socket::Create()]", GetError());
                return mess;
        }
...
char * Socket::Select(HWND handle, UINT msg, long events)
{
        if (WSAAsyncSelect(sock, handle, msg, events)) {
                sprintf_s(mess, "Error - %s - in [Socket::Select()]", GetError());
                return mess;
        }
...
char *Socket::Connect(const char *addr, WORD port)
{
        ...
        if ((saddr.sin_addr.S_un.S_addr = inet_addr(addr)) == INADDR_NONE) {        //name -> IP
                ...
                if (host) {
                        ...
                        strcpy_s(hostname, host->h_name);
                } else {
                        sprintf_s(mess, "Error - %s - in [Socket::Connect() gethostbyname ]", GetError());
                        return mess;
                }
        } else {    //IP -> name
        ...
        if (WSAConnect(sock, (sockaddr *)&saddr, sizeof(saddr), 0, 0, 0, 0)) {
                if (err = GetError()) {
                        sprintf_s(mess, "Error - %s - in [Socket::WSAConnect() WSAConnect ]", err);
                        return mess;
                }
        }
...
char *Socket::Listen(WORD port)
{
        ...
        if (bind(sock, (sockaddr *)&saddr, sizeof(saddr))) {
                sprintf_s(mess, "Error - %s - in [Socket::Listen() bind ]", GetError());
                return mess;
        }
        if (listen(sock, 10)) {
                sprintf_s(mess, "Error - %s - in [Socket::Listen() listen ]", GetError());
                return mess;
        }
...
char *Socket::Accept()
{
        ...
        if (server == INVALID_SOCKET) {
                sprintf_s(mess, "Error - %s - in [Socket::Listen() accept ]", GetError());
                return mess;
        }
...
char *Socket::Disconnect(SOCKET s)
{
        if (s) {
                if (res = closesocket(s)) {
                        sprintf_s(mess, "Error - %s - in [Socket::Disconnect() closesocket ]", GetError());
                        return mess;
                } else
                        return 0;
        }
...
char *Socket::getaddr(SOCKET s)
{
        for (int i = 0; i < active_connections.GetCount(); i++)
                if (((connections *)active_connections[i])->serv == s) {
                        sprintf_s(mess, "%s", inet_ntoa(((connections *)active_connections[i])->clntaddr.sin_addr));
                        return mess;
                }

        sprintf_s(mess, "sock %d not found in list?", s);
        return mess;
}
...
char *Socket::getaddr(int index)
{
        for (int i = 0; i < getclientsnum(); i++)
                if (index == i) {
                        sprintf_s(mess, "%s/%d", inet_ntoa(((connections *)active_connections[i])->clntaddr.sin_addr),
                                ntohs(((connections *)active_connections[i])->clntaddr.sin_port));
                        return mess;
                }

        sprintf_s(mess, "index %d not found in list?", index);
        return mess;
}
...
char *Socket::Read(SOCKET s)
{
        ...
        if (res = ioctlsocket(s, FIONREAD, &lastread)) {
                sprintf_s(mess, "Error - %s - in [Socket::Read() ioctlsocket ]", GetError());
                return mess;
        }
        ...
        if (res == SOCKET_ERROR) {
                sprintf_s(mess, "Error - %s - in [Socket::Read() recv ]", GetError());
                return mess;
        }
...
char *Socket::Send(char *message, SOCKET s)
{
        ...
        if (res == SOCKET_ERROR) {
                sprintf_s(mess, "Error - %s - in [Socket::Send() send ]", GetError());
                return mess;
        }
