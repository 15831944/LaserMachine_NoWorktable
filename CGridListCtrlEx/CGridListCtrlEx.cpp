...
bool CGridListCtrlEx::OnEditComplete(int nRow, int nCol, CWnd* pEditor, LV_DISPINFO* pLVDI)
{
	...
	if (txtEdit)
	{
		double fDelta, fPre, fScannerRgn;
		fScannerRgn = 1.1 * ReadScannerCaliRegion();
		LPWSTR strNew = pLVDI->item.pszText;
		double fNew = _ttof(strNew);
		fPre = fNew;

		if (nCol == 1 && nRow != GetItemCount() - 2)
			fPre = _ttof(GetItemText(nRow + 1, nCol));
		else if (nRow == GetItemCount() - 1 && nCol != 2)
			fPre = _ttof(GetItemText(nRow, nCol - 1));
		else if (nRow == GetItemCount() - 1 && nCol == 1)
			return false;

		fDelta = fNew - fPre;
		if (fDelta < 0 || fDelta > fScannerRgn)
		{
			AfxMessageBox(_T("分格单元超出加工幅面"));
			return false;
		}
	}
